class PCamDataset(Dataset):
    ...
    def __init__(self, root_dir, mode='train', label_filename=None, transform=None):
        """
        Args:
        ...
        if label_filename:
            label_df = pd.read_csv(label_filename).set_index('id', verify_integrity=True)
            self.labels = [int(label_df.loc[f.stem, 'label']) for f in filepaths]
